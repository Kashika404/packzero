...
//     e.preventDefault();
//     console.log('Attempting to call the register API with:', { email, password });
//     try {
//     //   await axios.post('http://localhost:8888/api/auth/register', { email, password });
//     await register(email,password);
//       console.log('API call successful! Navigating to login...');
...
//           <button type="submit" className="w-full bg-purple-600 text-white py-3 rounded-md hover:bg-purple-700">Register</button>
//         </form>
...
function Register() {
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const { register } = useAuth(); // This will now correctly receive the function
  const navigate = useNavigate();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // This will now correctly catch errors from the backend, like "User already exists".
      const errorMessage = error.response?.data?.message || "An unknown registration error occurred.";
