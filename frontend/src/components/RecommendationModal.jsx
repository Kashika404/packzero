...
function RecommendationModal({ isOpen, onClose, results, fromAddress, toAddress }) {
  ...
  const shipmentToVisualize = results.recommendedPackages[selectedPackageIndex];
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex border-b mb-4">
            <button onClick={() => setView('rates')} className={`py-2 px-4 font-semibold ${view === 'rates' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'}`}>
                Shipping Rates
            </button>
            <button onClick={() => setView('3d')} className={`py-2 px-4 font-semibold ${view === '3d' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500'}`}>
                3D View
            </button>
        </div>
        ...
        {view === 'rates' ? (
              ...
              {results.recommendedPackages.map((shipment, index) => {
                ...
                return (
                  <div key={index} className="border border-gray-200 rounded-lg p-4">
                    <h4 className="text-lg font-semibold text-blue-700">Box {index + 1}: <span className="font-bold text-gray-900">{shipment.packaging.name}</span></h4>
                    <p className="text-sm text-gray-600">Contains: {productNames.join(', ')}</p>
                    <button onClick={() => fetchShippingRates(shipment.packaging, index)} className="w-full mt-3 px-4 py-2 bg-indigo-600 text-white font-semibold rounded-md hover:bg-indigo-700" disabled={isLoadingRates}>
          ...
          <div>
            <h3 className="text-xl font-semibold text-gray-800 mb-2">3D View</h3>
           
            {results.recommendedPackages.length > 1 && (
              <div className="flex gap-2 mb-4 border-b pb-4">
                <span className="font-semibold self-center">Select package:</span>
                {results.recommendedPackages.map((_, index) => (
                  <button 
                    key={index} 
                    onClick={() => setSelectedPackageIndex(index)} 
                    className={`px-3 py-1 rounded-md text-sm font-semibold ${selectedPackageIndex === index ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'}`}
                  >
                    Box {index + 1}
                  </button>
                ))}
            ...
            {shipmentToVisualize && <Visualization shipment={shipmentToVisualize} />}
          </div>
        )}
